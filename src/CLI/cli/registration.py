...
# import sys
# sys.path.append(home/shourya/poet2/sawtooth-poet2/src/CLI/signing)
from signing.sawtooth_signing import create_context
from signing.sawtooth_signing import CryptoFactory
from signing.sawtooth_signing import ParseError
from signing.sawtooth_signing.secp256k1 import Secp256k1PrivateKey

import sawtooth_sdk.protobuf.transaction_pb2 as txn_pb
import sawtooth_sdk.protobuf.batch_pb2 as batch_pb
#  
from protos import validator_registry as vr_pb
   
from cli import config 
from cli.exceptions import CliException
from cli.poet_enclave_module_wrapper import PoetEnclaveModuleWrapper
from poet2_consensus.signup_info import SignupInfo
# # from sawtooth_poet.poet_consensus.poet_key_state_store \
# #     import PoetKeyState
# # from sawtooth_poet.poet_consensus.poet_key_state_store \
# #     import PoetKeyStateStore
# #from sawtooth_poet.journal.block_wrapper import 
from state.settings_view import SettingsView
#import sawtooth_poet_common.protobuf.validator_registry_pb2 as vr_pb

NULL_BLOCK_IDENTIFIER="0000000000000"
...
def add_registration_parser(subparsers):
    ...
    add_create_parser(registration_sub)
...
def do_create(args):
    ...
    header = \
        txn_pb.TransactionHeader(
            ...
            family_name='validator_registry',
            family_version='1.0',
...
def _read_signer(key_filename):
    ...
    if key_filename is None:
        raise CliException('No key provided')

    filename = _get_keyfile(key_filename)
...
def _send_to_restapi(suffix,data=None,contentType=None):
    '''Send a REST command to the Validator via the REST API.'''
    _rest_url= 'http://rest-api:8008'
   
    if _rest_url.startswith("http://"):
        url = "{}/{}".format(_rest_url, suffix)
    else:
        url = "http://{}/{}".format(_rest_url, suffix)
...
def _get_keyfile(keyName):
    '''Get the private key for a customer.'''
    home = os.path.expanduser("~")
    key_dir = os.path.join(home, ".sawtooth", "keys")

    return '{}/{}.priv'.format(key_dir, keyName)
